...
RETURNS table(nome_UF varchar(2), soma_div numeric, qtd_devedores bigint, qtd_div bigint, soma_acoes numeric, qtd_acoes bigint) AS $$
	BEGIN
		RETURN QUERY 
		SELECT estados.nome_UF as nome_UF, coalesce(sum_div, 0) as soma_div, coalesce(qnt_devedores, 0) as qtd_devedores, coalesce(qnt_div, 0) as qtd_div, coalesce(sum_acoes, 0) as soma_acoes, coalesce(qnt_acoes, 0) as qtd_acoes
		FROM UFs as estados
		LEFT JOIN
...
RETURNS table(cpf varchar(11), nome varchar(50), data_do_nascimento date, estado_civil varchar(15), qtd_dividas bigint, qtd_processo bigint) AS $$
	BEGIN 
		...
		LEFT JOIN 
			...
			WHERE extract(year from data) >= d1 AND extract(year from data) <= d2
